...
const mapFallbackProduct = (fp) => {
  ...
  return {
    ...
    is_new_arrival: Boolean(fp.is_new_arrival ?? fp.isNew ?? false),
    isNew: Boolean(fp.is_new_arrival ?? fp.isNew ?? false),
      ...
      if (prodRes.ok) {
        ...
        const mappedProducts = productsList.map(bp => {
          ...
          return {
            ...
            is_new_arrival: Boolean(bp.is_new_arrival ?? bp.isNew ?? matchedFallback?.is_new_arrival ?? false),
            isNew: Boolean(bp.is_new_arrival ?? bp.isNew ?? matchedFallback?.is_new_arrival ?? false),
            show_on_homepage_new_arrivals: Boolean(bp.show_on_homepage_new_arrivals ?? matchedFallback?.show_on_homepage_new_arrivals ?? false),
            homepage_new_arrival_position: bp.homepage_new_arrival_position ? Number(bp.homepage_new_arrival_position) : (matchedFallback?.homepage_new_arrival_position ? Number(matchedFallback.homepage_new_arrival_position) : null),
            moment: finalMoment,
        ...
        FALLBACK_PRODUCTS.forEach(fp => {
          if (!existingSlugs.has(cleanSlug(fp.slug)) && !existingIds.has(String(fp.id))) {
            ...
            mergedProducts.push({
              ...
              is_new_arrival: Boolean(fp.is_new_arrival ?? fp.isNew ?? false),
              isNew: Boolean(fp.is_new_arrival ?? fp.isNew ?? false),
              show_on_homepage_new_arrivals: Boolean(fp.show_on_homepage_new_arrivals ?? false),
              homepage_new_arrival_position: fp.homepage_new_arrival_position ? Number(fp.homepage_new_arrival_position) : null,
              moment: fp.moment || 'boardroom',
              moments: Array.isArray(fp.moments) ? fp.moments : [fp.moment || 'boardroom'],
              momentName: fp.momentName || 'The Boardroom Edit',
              moment_name: fp.momentName || 'The Boardroom Edit',
