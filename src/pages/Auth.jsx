...
const Auth = () => {
  ...
  const [authMode, setAuthMode] = useState(() => (modeParam === "register" ? "register" : "password")); // 'password' | 'register' | 'forgot-password'
  
  // Registration Multi-Step State: 1 = Details, 2 = Verify OTP, 3 = Password & Confirm
  const [registerStep, setRegisterStep] = useState(1);
  // Forgot Password Multi-Step State: 1 = Email, 2 = Verify OTP, 3 = New Password & Confirm
  ...
  const handleSendResetOtp = async (isResend = false) => {
    ...
    try {
      ...
      if (res?.success) {
        ...
        if (isResend) {
          toast.success("New password reset code dispatched.");
        } else {
          toast.success(`Password reset code sent to ${cleanEmail}.`);
          setForgotStep(2);
        }
      }
    } catch (err) {
  ...
  const handleVerifyResetOtp = async (e) => {
    ...
    setIsLoading(true);
    try {
      const res = await verifyResetOtp(email.trim().toLowerCase(), cleanOtp);
      if (res?.success && res.resetToken) {
        setResetToken(res.resetToken);
        toast.success("Authorization code verified! Please set your new password.");
        setForgotStep(3);
      }
    } catch (err) {
      toast.error(err.message || "Invalid verification code.");
  ...
  const handleCompletePasswordReset = async (e) => {
    ...
    if (!resetToken) {
      toast.error("Password reset session expired. Please request a new code.");
      setForgotStep(1);
      return;
    }
    ...
    setIsLoading(true);
    try {
      const res = await resetPassword(email.trim().toLowerCase(), password, resetToken);
      if (res?.success) {
        ...
        setResetToken("");
        setForgotStep(1);
        setAuthMode("password"); // Smooth redirect back to Sign In
      }
    } catch (err) {
      ...
      <div className="max-w-[460px] w-full bg-[#FAF8F5] border border-[#E8E4DC] p-7 sm:p-10 relative shadow-[0_12px_40px_rgba(0,0,0,0.04)] transition-all duration-300 z-10">
              ...
              <div className="space-y-1">
                <p className="text-xs uppercase tracking-[0.3em] font-body text-[#111113] font-medium animate-pulse">
                  {authMode === "register" && registerStep === 1 ? "Sending Authorization Code..." : 
                   ...
                   authMode === "forgot-password" && forgotStep === 1 ? "Sending Reset Code..." :
                   authMode === "forgot-password" && forgotStep === 2 ? "Verifying Authorization Code..." :
                   authMode === "forgot-password" && forgotStep === 3 ? "Updating Atelier Password..." :
                   "Authenticating..."}
                </p>
        ...
        <button
          onClick={() => {
            ...
            } else if (authMode === "forgot-password" && forgotStep > 1) {
          ...
          <div className="mb-6 p-4 bg-[#F4EFE6] border border-[#C2922E]/30 text-left">
            ...
            <p className="text-xs text-[#555560] font-light leading-relaxed">
              {authMode === "register"
                ...
                : authMode === "forgot-password"
                ? "Reset your password to access your SUKO account and complete your order."
        ...
        {authMode !== "forgot-password" ? (
          ...
          /* 3-Step Indicator for Password Reset */
          <div className="mb-6 flex items-center justify-between border-b border-[#EAE6DF] pb-3 text-center">
            <div className={`flex items-center gap-1.5 text-[9.5px] uppercase tracking-[0.16em] font-mono transition-colors ${
              forgotStep === 1 ? "text-[#C2922E] font-bold" : forgotStep > 1 ? "text-[#111113] font-medium" : "text-[#A3A096]"
            }`}>
              <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] ${
                forgotStep >= 1 ? "bg-[#111113] text-white" : "bg-[#DDD8CE] text-[#777782]"
              }`}>1</span>
              <span>Email</span>
            </div>
            ...
            <div className={`flex items-center gap-1.5 text-[9.5px] uppercase tracking-[0.16em] font-mono transition-colors ${
              forgotStep === 2 ? "text-[#C2922E] font-bold" : forgotStep > 2 ? "text-[#111113] font-medium" : "text-[#A3A096]"
            }`}>
              <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] ${
                forgotStep >= 2 ? "bg-[#111113] text-white" : "bg-[#DDD8CE] text-[#777782]"
              }`}>2</span>
            ...
            <div className={`flex items-center gap-1.5 text-[9.5px] uppercase tracking-[0.16em] font-mono transition-colors ${
              forgotStep === 3 ? "text-[#C2922E] font-bold" : "text-[#A3A096]"
            }`}>
              <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] ${
                forgotStep === 3 ? "bg-[#111113] text-white" : "bg-[#DDD8CE] text-[#777782]"
              }`}>3</span>
              <span>Password</span>
            </div>
          </div>
        ...
        {/* ========================================================================= */}
        {/* FORGOT PASSWORD: STEP 1 (ENTER EMAIL & DISPATCH OTP)                     */}
        {/* ========================================================================= */}
        {authMode === "forgot-password" && forgotStep === 1 && (
          <form onSubmit={(e) => { e.preventDefault(); handleSendResetOtp(false); }} className="space-y-5 font-body">
            <p className="text-xs text-[#555560] font-light leading-relaxed">
              Enter your registered SUKO email address below. We will send a secure 6-digit authorization code to reset your password.
            </p>
            ...
            <button
              ...
              onClick={() => handleModeSwitch("password")}
              disabled={isLoading}
              className="w-full text-center py-2 text-[10px] uppercase tracking-[0.20em] text-[#777782] hover:text-[#111113] transition-colors cursor-pointer"
            >
              Back to Sign In
            </button>
          </form>
        ...
        {/* ========================================================================= */}
        {/* FORGOT PASSWORD: STEP 2 (VERIFY 6-DIGIT OTP WITH COUNTDOWN RESEND)        */}
        {/* ========================================================================= */}
        {authMode === "forgot-password" && forgotStep === 2 && (
          <form onSubmit={handleVerifyResetOtp} className="space-y-6 font-body">
            <div className="bg-[#F5F2EB] p-3.5 border border-[#EAE6DF] rounded-xs text-left">
              <div className="flex items-center justify-between gap-2 mb-1">
                <span className="text-[9.5px] uppercase tracking-[0.20em] text-[#C2922E] font-medium">
                  RESET CODE SENT
            ...
            <div className="space-y-2 pt-2">
              ...
              <button
                ...
                onClick={() => setForgotStep(1)}
                disabled={isLoading}
                className="w-full text-center py-2.5 text-[10px] uppercase tracking-[0.20em] text-[#777782] hover:text-[#111113] transition-colors cursor-pointer"
              >
                Back to Email
              </button>
            </div>
        ...
        {/* ========================================================================= */}
        {/* FORGOT PASSWORD: STEP 3 (CREATE & CONFIRM NEW PASSWORD)                   */}
        {/* ========================================================================= */}
        {authMode === "forgot-password" && forgotStep === 3 && (
          <form onSubmit={handleCompletePasswordReset} className="space-y-5 font-body">
            {/* Verified Email Banner */}
            <div className="flex items-center justify-between bg-emerald-50 border border-emerald-200 px-3.5 py-2.5 rounded-xs">
              <div className="flex items-center gap-2 text-emerald-800 text-xs font-medium">
                <CheckCircle2 size={14} className="text-emerald-600 shrink-0" />
            ...
            <button
              ...
              className="w-full bg-[#111113] text-white py-4 text-[11px] uppercase tracking-[0.26em] font-medium hover:bg-[#C2922E] transition-all duration-300 flex items-center justify-center gap-2.5 disabled:opacity-50 shadow-sm cursor-pointer mt-3"
            >
              <span>Reset Password &amp; Proceed to Sign In</span>
            </button>
